...
st.set_page_config(page_title="Manual Neural Net from Scratch - Classification", layout="centered")

st.title("Neural Network Built From Scratch (NumPy Only) - Classification")
st.caption(
    "A 2 -> 2 -> 1 network (no ML framework) predicting placement (0/1) from CGPA and profile score. "
    "Forward pass uses sigmoid activation, and backpropagation is hand-written — this app trains it live."
)
...
with col1:
    epochs = st.slider("Epochs", min_value=1, max_value=500, value=50, step=1)
with col2:
    lr = st.select_slider(
        "Learning rate",
...
if "history" in st.session_state:
    ...
    with pcol1:
        st.markdown("**W1** (input to hidden)")
        st.write(parameters["W1"])
        st.markdown("**b1**")
        st.write(parameters["b1"])
    with pcol2:
        st.markdown("**W2** (hidden to output)")
    ...
    result_df["predicted_prob"] = np.round(preds, 3)
    result_df["predicted_class"] = (result_df["predicted_prob"] >= 0.5).astype(int)
    st.dataframe(result_df, use_container_width=True)

    accuracy = (result_df["predicted_class"] == result_df["placed"]).mean()
    ...
    st.success(f"Predicted probability: {prob:.3f} -> {label}")
else:
    st.info("Adjust the data/hyperparameters above and click **Train network** to begin.")

st.divider()
st.caption(
    "Built to demonstrate manual implementation of forward propagation (sigmoid activation) and "
    "backpropagation (chain rule, gradient descent) without using PyTorch/TensorFlow."
)